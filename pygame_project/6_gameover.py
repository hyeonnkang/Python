...
# 게임 종료 메시지
# TimeOut(시간 초과), Mission Complete(성공), Game Over (캐릭터 공에 맞음, 실패)
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
                    ...
                    balls.append(
                        {
                            "pos_x": ball_pos_x
                            + (ball_width / 2)
                            - (small_ball_width / 2),  # 공의 x 좌표
                            "pos_y": ball_pos_y
                            + (ball_width / 2)
                            - (small_ball_height / 2),  # 공의 y 좌표
        ...
        break  # 안쪽 for 문에서 break 를 만나면 여기로 진입가능. 2중 for 문을 한번에 탈출
    ...
    timer = game_font.render(
        "Time : {}".format(int(total_time - elapsed_time)), True, (255, 255, 255)
    )
